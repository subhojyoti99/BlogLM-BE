...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS blogs (
            ...
            search_id TEXT,
            blog_type TEXT CHECK(blog_type IN ('regular', 'blended')) DEFAULT 'regular',
            title TEXT NOT NULL,
            subtitle TEXT,
...
def save_blog_outcome(
    search_id: str,
    blog_type: str,  # "regular" or "blended"
    title: str,
    subtitle: str,
    content_json: dict,
    summary: str,
    ...
    individual_summaries: Optional[List[str]] = None
):
    """Save regular or blended blog outcome to unified table."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    cursor.execute('''
        INSERT INTO blogs (
            search_id, blog_type, title, subtitle, content_json, summary,
            linkedin_version, linkedin_content, docx_filename,
            topic_indices, individual_summaries, tone, word_count, tags, seo_meta
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
        search_id,
        blog_type,
        ...
        linkedin_content,
        docx_filename,
        json.dumps(topic_indices) if topic_indices else None,
        json.dumps(individual_summaries) if individual_summaries else None,
        tone,
        word_count,
...
def get_all_blogs(limit: int = 50, offset: int = 0):
    """Retrieve all blogs (regular + blended) with pagination"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    cursor.execute('''
        SELECT id, search_id, blog_type, title, subtitle, docx_filename, tone, 
               word_count, linkedin_version, created_at
        FROM blogs 
...
def get_blog_by_id(blog_id: int):
    """Retrieve complete blog data by ID (supports both regular and blended)"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    cursor.execute('''
        SELECT 
            id, search_id, blog_type, title, subtitle, content_json, summary,
            linkedin_version, linkedin_content, docx_filename,
            topic_indices, individual_summaries, tone, word_count, tags,
            seo_meta, created_at, updated_at
        FROM blogs 
        WHERE id = ?
    ''', (blog_id,))
    ...
    # Get Instagram posts for this blog (if applicable)
    cursor.execute('''
    ...
    blog = {
        ...
        'linkedin_content': row[8],
        'docx_filename': row[9],
        'topic_indices': json.loads(row[10]) if row[10] else [],
        'individual_summaries': json.loads(row[11]) if row[11] else [],
        'tone': row[12],
        'word_count': row[13],
...
class UnifiedBlogRequest(BaseModel):
    ...
    topic_indices: List[int] = Field(..., description="List of topic indices (single or multiple)")
...
async def generate_blog_unified(request: UnifiedBlogRequest):
    """
    Generate a blog for one or multiple topics.
    If one topic is selected → regular blog.
    If multiple topics are selected → blended blog.
    """
    # Retrieve topics
    ...
    for idx in request.topic_indices:
        if idx >= len(topics):
            raise HTTPException(status_code=400, detail=f"Invalid topic index: {idx}")

    selected_topics = [topics[i] for i in request.topic_indices]
    blog_type = "regular" if len(selected_topics) == 1 else "blended"

    # --- Summarization Step ---
    summaries = [summarize_blog(t["url"]) for t in selected_topics]

    # --- Blending or Direct ---
    if blog_type == "regular":
        final_summary = summaries[0]
        final_title = selected_topics[0]["title"]
    else:
        final_summary = blend_multiple_blogs(
            summaries,
            [topic["title"] for topic in selected_topics]
        )
        partial_titles = [topic["title"][:30] for topic in selected_topics[:3]]
        final_title = f"Comprehensive Analysis: {', '.join(partial_titles)}"
        if len(selected_topics) > 3:
            final_title += " and more"

    # --- Generate Blog ---
    blog = generate_blog(final_title, final_summary, request.tone, request.word_target)
    linkedIn_version = linkedIn_blog(blog)

    # --- Export DOCX ---
    safe_filename = (
        f"{blog_type}_blog_{uuid.uuid4().hex[:8]}.docx"
        if blog_type == "blended"
        else blog.title.replace(" ", "_").replace("/", "_") + ".docx"
    )
    ...
    blog_id = save_blog_outcome(
        ...
        docx_filename=safe_filename,
        tone=request.tone,
        word_count=request.word_target,
        tags=blog.tags,
        seo_meta=blog.seo_meta,
        blog_type=blog_type,
        topic_indices=request.topic_indices if blog_type == "blended" else None,
        individual_summaries=summaries if blog_type == "blended" else None
    )

    # --- Save LinkedIn Post ---
    save_linkedin_post(blog_id, blog_type, linkedIn_version)
